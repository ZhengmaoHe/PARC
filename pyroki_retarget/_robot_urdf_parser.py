...
class LinkInfo:
    ...
    parent_link_indices: Int[Array, " n_links"]
...
class RobotURDFParser:
    ...
    def parse(urdf: yourdfpy.URDF) -> tuple[JointInfo, LinkInfo]:
        ...
        # Compute parent_link_indices
        name_to_idx = {name: i for i, name in enumerate(link_name_list)}
        parent_link_idx_list = []
        for parent_joint_idx in parent_joint_idx_list:
            if parent_joint_idx == -1:
                parent_link_idx_list.append(-1)
            else:
                parent_link_name = urdf.joint_map[joint_name_list[parent_joint_idx]].parent
                parent_link_idx_list.append(name_to_idx[parent_link_name])
        ...
        link_info = LinkInfo(
            ...
            parent_link_indices=jnp.array(parent_link_idx_list, dtype=jnp.int32),
        )
        assert link_info.parent_joint_indices.shape == (link_info.num_links,)
        assert link_info.parent_link_indices.shape == (link_info.num_links,)

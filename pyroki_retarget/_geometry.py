...
class CollGeom(abc.ABC):
    ...
    size: Float[Array, "*batch shape_dim"]  # type: ignore
    ...
    def transform_from_wxyz_position(
        self,
        wxyz: Float[ArrayLike, "*batch 4"],  # type: ignore
        position: Float[ArrayLike, "*batch 3"],  # type: ignore
    ) -> Self:
        """Left-multiples geometry's pose with an SE(3) transformation.
...
class HalfSpace(CollGeom):
    ...
    @property
    def normal(self) -> Float[Array, "*batch 3"]:  # type: ignore
    ...
    def from_point_and_normal(
        point: Float[ArrayLike, "*batch 3"], normal: Float[ArrayLike, "*batch 3"]  # type: ignore
    ) -> HalfSpace:
...
class Sphere(CollGeom):
    ...
    def from_center_and_radius(
        center: Float[ArrayLike, "*batch 3"], radius: Float[ArrayLike, "*batch"]  # type: ignore
    ) -> Sphere:
...
class Capsule(CollGeom):
    ...
    @property
    def axis(self) -> Float[Array, "*batch 3"]:  # type: ignore
    ...
    def from_radius_height(
        ...
        position: Float[ArrayLike, "*batch 3"] | None = None,  # type: ignore
        wxyz: Float[ArrayLike, "*batch 4"] | None = None,  # type: ignore
    ) -> Capsule:
...
class Heightmap(CollGeom):
    ...
    height_data: Float[Array, "*batch H W"]  # type: ignore
    ...
    def from_height_data(
        height_data: Float[ArrayLike, "*batch H W"],  # type: ignore
    ...
    def _get_vertices_local(self) -> Float[Array, "*batch H*W 3"]:  # type: ignore
    ...
    def _create_one_mesh(self, index: tuple) -> trimesh.Trimesh:
        """Create a single trimesh object from height data at a given index with vertical walls instead of slopes."""
        pose_i: jaxlie.SE3 = jax.tree.map(lambda x: x[index], self.pose)
        height_data_i: Float[Array, "H W"] = self.height_data[index]  # type: ignore
        ...
        # Compute scaled heights (NumPy for Trimesh compatibility)
        heights = onp.array(height_data_i) * height_scale_i

        # Generate grid positions (centered, as before)
        x = onp.arange(cols) * x_scale_i - (cols - 1) * x_scale_i / 2.0
        y = onp.arange(rows) * y_scale_i - (rows - 1) * y_scale_i / 2.0
        xx, yy = onp.meshgrid(x, y)

        vertices = []
        faces = []

        # Base Z: global minimum height for enclosure
        base_z = onp.min(heights) if onp.any(heights) else 0.0

        # Helper to add quad as two triangles (with front and back)
        def add_quad_faces(v0, v1, v2, v3):
            # Front
        ...
        # For each cell (r, c), create top and conditional sides
        for r in range(rows):
            for c in range(cols):
                h = heights[r, c]

                # Skip if height is at base (no volume)
                if h <= base_z:
                    continue
                ...
                # Top face (flat)
                add_quad_faces(v_base+0, v_base+1, v_base+2, v_base+3)

                # Side walls: check neighbors and add vertical quads from lower to higher
                # North (top, r-1)
                if r > 0:
                    h_n = heights[r-1, c]
                    z_low = max(base_z, min(h, h_n))
                    z_high = max(h, h_n)
                    if z_high > z_low:
                        # Add 4 vertices for this wall at z_low and z_high
                        w_base = len(vertices)
                        vertices.extend([
                            [x0, y0, z_low], [x1, y0, z_low],
                            [x0, y0, z_high], [x1, y0, z_high]
                        ])
                        add_quad_faces(w_base+0, w_base+1, w_base+3, w_base+2)
                else:
                    # Edge: full wall to base_z
                    w_base = len(vertices)
                    vertices.extend([
                ...
                if r < rows - 1:
                    h_s = heights[r+1, c]
                    z_low = max(base_z, min(h, h_s))
                    z_high = max(h, h_s)
                    if z_high > z_low:
                        w_base = len(vertices)
                        vertices.extend([
                            [x0, y1, z_low], [x1, y1, z_low],
                            [x0, y1, z_high], [x1, y1, z_high]
                        ])
                        add_quad_faces(w_base+0, w_base+1, w_base+3, w_base+2)
                else:
                    # Edge
                ...
                if c > 0:
                    h_w = heights[r, c-1]
                    z_low = max(base_z, min(h, h_w))
                    z_high = max(h, h_w)
                    if z_high > z_low:
                        w_base = len(vertices)
                        vertices.extend([
                            [x0, y0, z_low], [x0, y1, z_low],
                            [x0, y0, z_high], [x0, y1, z_high]
                        ])
                        add_quad_faces(w_base+0, w_base+1, w_base+3, w_base+2)
                else:
                    # Edge
                ...
                if c < cols - 1:
                    h_e = heights[r, c+1]
                    z_low = max(base_z, min(h, h_e))
                    z_high = max(h, h_e)
                    if z_high > z_low:
                        w_base = len(vertices)
                        vertices.extend([
                            [x1, y0, z_low], [x1, y1, z_low],
                            [x1, y0, z_high], [x1, y1, z_high]
                        ])
                        add_quad_faces(w_base+0, w_base+1, w_base+3, w_base+2)
                else:
                    # Edge
                    w_base = len(vertices)
                    vertices.extend([
                        [x1, y0, base_z], [x1, y1, base_z],
                        [x1, y0, h], [x1, y1, h]
                    ])
                    add_quad_faces(w_base+0, w_base+1, w_base+3, w_base+2)

        # Add a base plane if needed (full grid at base_z)
        # (Optional: add code here if you want a bottom plane)

        # Create mesh
        if not vertices:
            return trimesh.Trimesh()

        heightmap_mesh = trimesh.Trimesh(vertices=onp.array(vertices), faces=onp.array(faces))

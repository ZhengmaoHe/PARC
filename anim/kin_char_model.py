...
class Geom:
    def __init__(self, shape_type, offset, dims, device, quat=None, radius=None, mesh_name=None):
...
class KinCharModel():
    ...
    def init(self, body_names, parent_indices, local_translation, local_rotation, joints, geoms=None):
    ...
    def apply_scales_to_local_translation(self, x_scale, y_scale, z_scale):
        self._local_translation = self._original_local_translation.clone()
        self._local_translation[..., 0] *= x_scale
        self._local_translation[..., 1] *= y_scale
        self._local_translation[..., 2] *= z_scale
        return
    ...
    def get_copy(self, new_device):
        ...
        char_model.init(body_names, parent_indices, local_translation, local_rotation, joints, geoms)
        return char_model
        ...
        def get_geom(xml_node):
            ...
            for geom_node in xml_node.findall("geom"):
                ...
                if shape_type == GeomType.SPHERE or shape_type == GeomType.BOX:
                    ...
                    geom = Geom(shape_type, offset, dims, self._device, quat=quat)
                    ...
                    quat = get_quat(geom_node)
                    #print("radius:", radius)
                    geom = Geom(shape_type, offset, dims, self._device, quat=quat, radius=radius)
                    ...
                    geom = Geom(shape_type, offset, dims, self._device, 
                                radius=radius, mesh_name=mesh_name, quat=quat)
                else:
                    assert False
...
{:s}
    ...
    def extract_frame_data(self, motion_frames):
        ...
        joint_dof = motion_frames[..., 6:6+self.get_dof_size()]
        joint_rot = self.dof_to_rot(joint_dof)
